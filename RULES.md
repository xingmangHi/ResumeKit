...
`prospect/` 下交付的 HTML 打印成 PDF **必须**默认为 **1 页**（A4），除非：

- 第 4.1 轮「目标页数」用户**显式**选了「2 页 / 不限」
- 源文件内容物理上确实无法压缩到 1 页（且已**先**与用户确认）

实现方式（按优先级）：

1. **首选**：调整 `@media print` 内的样式压缩垂直空间
   - 减小 `@page` 边距（上下最小不低于 `0.5cm`）
   - 减小 body / 章节标题 / 条目 / 列表的字号与行高
   - 减小 `.section` 间距与 `.item-list > div` 行高
2. **次选**：合并 / 删除冗余字段（如可省略的副标题、装饰性文字）— **必须先**与用户确认，不得擅自删字
3. **禁止**：用 CSS `transform: scale` 缩放内容裁切边缘
4. **兜底**：经 1、2 仍超过 1 页时，**先**告知用户当前实测页数，由用户决定：接受多页 / 砍内容 / 改 2 页布局
5. **验证**：样式调整后必须用 Edge headless 渲染一次 PDF，确认页数符合预期（不能只让用户打印自查）

> 「内容不是特别多」是用户的默认预期；只要源文件内容在 1 页内能装下，**必须**装下，不许默默出 2 页。
